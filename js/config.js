...
const LEVELS = [
  // Level 0 — TUTORIAL
  {
    ...
    gaps: [{x:28,w:3}],
    platforms: [
      // Wide flat opening for movement/jump teaching
      {x:5,y:8,w:8,t:'brick'},
      // A platform gap to practice jump over
      {x:14,y:8,w:6,t:'brick'},
      // Raised platform for dash teaching
      {x:22,y:6,w:6,t:'brick'},
      // Mid-air platform cluster for homing
      {x:35,y:6,w:5,t:'qblock'},{x:42,y:7,w:5,t:'qblock'},
      // Brick blocks for turbo dash-break teaching (last area)
  ...
  // Level 1 — ~3× Mario 1-1 length (flag at tile 580)
  {
    ...
    goombas: [
      ...
      {x:455,pl:445,pr:470},{x:472,pl:462,pr:488},
      {x:505,pl:495,pr:520},{x:528,pl:515,pr:542},
      {x:558,pl:548,pr:572},{x:570,pl:560,pr:578},
    ],
    flyers: [
      {x:18,fy:6*TILE,pw:7},{x:31,fy:7*TILE,pw:7},{x:45,fy:6*TILE,pw:7},
      {x:72,fy:6*TILE,pw:7},{x:85,fy:7*TILE,pw:7},{x:98,fy:6*TILE,pw:7},
      {x:138,fy:6*TILE,pw:7},{x:150,fy:7*TILE,pw:7},{x:162,fy:6*TILE,pw:7},
      ...
      {x:430,fy:6*TILE,pw:7},{x:442,fy:7*TILE,pw:7},{x:454,fy:6*TILE,pw:7},
    ],
  },
